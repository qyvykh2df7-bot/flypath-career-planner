      ...
      {items.map((item) => {
        ...
        return (
          <article key={item.label} className={plannerMetricCard}>
            ...
            <p className="mt-2.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">
              {item.label}
            </p>
            <p className="mt-0.5 text-xl font-semibold text-[#0f1a33]">{item.value}</p>
            <p className="mt-1.5 text-[12px] leading-snug text-slate-500">{item.hint}</p>
          </article>
        );
      })}
    </section>
