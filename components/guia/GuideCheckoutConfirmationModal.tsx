...
export function GuideCheckoutConfirmationModal({ sessionId }: { sessionId: string | null }) {
  ...
  const verifyAndReadStatus = useCallback(async (): Promise<CareerPlannerCheckoutPresentationStatus> => {
    ...
    const access = await fetch("/api/commerce/guide/access", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sessionId }),
    });
    if (!access.ok) return "delayed";
    const response = await fetch("/api/commerce/guide/status", { cache: "no-store" });
    const payload: unknown = await response.json().catch(() => null);
    const nextStatus = typeof payload === "object" && payload !== null && "status" in payload
      ? (payload as { status?: unknown }).status : null;
  ...
  useEffect(() => {
    ...
    void poll();
    return () => { disposed = true; if (timeout) clearTimeout(timeout); previousFocus?.focus(); };
  }, [verifyAndReadStatus]);

  const close = () => { if (canCloseCareerPlannerCheckoutConfirmation(status)) window.location.assign("/guia-como-ser-piloto"); };
  ...
  const retry = async () => { setStatus("verifying"); setStatus(await verifyAndReadStatus().catch(() => "delayed" as const)); };
  const download = async () => {
    setIsDownloading(true); setDownloadError(null);
    try {
      const response = await fetch("/api/commerce/guide/download", { method: "POST" });
      if (!response.ok) throw new Error("Download unavailable");
      const url = URL.createObjectURL(await response.blob());
      const anchor = document.createElement("a");
      anchor.href = url; anchor.download = "como-ser-piloto.pdf"; document.body.appendChild(anchor); anchor.click(); anchor.remove(); URL.revokeObjectURL(url);
    } catch {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#0f1a33] px-6 py-12 text-white">
      ...
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="guide-checkout-title" aria-describedby="guide-checkout-description" aria-live="polite" tabIndex={0} onKeyDown={handleKeyDown} className="relative z-10 w-full max-w-lg rounded-xl border border-white/10 bg-[#16223f] p-7 shadow-[0_16px_48px_rgba(0,0,0,0.28)] outline-none sm:p-9">
        ...
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#d6ae4f]">Guía Cómo ser Piloto</p>
        <h1 id="guide-checkout-title" className="mt-3 text-2xl font-semibold">{title}</h1>
        <p id="guide-checkout-description" className="mt-3 text-sm leading-6 text-slate-300">{copy}</p>
        ...
        {!isVerifying ? <button type="button" onClick={close} className="mt-3 inline-flex min-h-11 w-full items-center justify-center rounded-lg border border-white/15 px-4 py-2 text-sm font-semibold text-slate-200 hover:border-white/30">Volver a la guía</button> : null}
      </section>
    </main>
  );
}
