-- =============================================================================
-- FlyPath: move new AeroComms Pro subscriptions to 5.99 EUR/month.
--
-- Stripe Prices are immutable. Keep the 7.37 EUR price as an archived FlyPath
-- catalog identity for subscriptions already created with it, and introduce a
-- separate active identity for all future Checkout Sessions.
-- =============================================================================
...
BEGIN
  ...
  IF v_product_id IS NULL THEN
    RAISE EXCEPTION 'AeroComms Pro product is unavailable'
      USING ERRCODE = '23514';
  END IF;

  SELECT id INTO v_legacy_price_id
  FROM public.product_prices
  WHERE product_id = v_product_id
    AND price_key = 'aerocomms_pro_monthly_eur'
    AND currency = 'EUR'
    AND unit_amount = 737
    AND billing_type = 'recurring'
    AND billing_interval = 'month'
    AND interval_count = 1
    AND stripe_product_id = 'prod_UvXKn9mQPp3G17'
    AND stripe_price_id = 'price_1TvgG4KuujVRKb0PkofwZMz7';

  IF v_legacy_price_id IS NULL THEN
    RAISE EXCEPTION 'AeroComms Pro legacy price is unavailable'
      USING ERRCODE = '23514';
  END IF;

  -- Only archive the legacy price for future FlyPath Checkout Sessions. Its
  -- immutable identity and every historical foreign key remain untouched.
  UPDATE public.product_prices
    SET is_active = false
  WHERE id = v_legacy_price_id;

  SELECT id INTO v_current_price_id
  FROM public.product_prices
  WHERE price_key = 'aerocomms_pro_monthly_eur_599';
  ...
  IF v_current_price_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      WHERE id = v_current_price_id
        ...
        AND interval_count = 1
        AND stripe_product_id = 'prod_UvXKn9mQPp3G17'
        AND stripe_price_id = 'price_1Tw6JqKuujVRKb0Pr4jCc5oQ'
    ) THEN
      RAISE EXCEPTION 'AeroComms Pro 5.99 EUR price conflicts with the closed catalog contract'
        USING ERRCODE = '23514';
    END IF;
  ...
  ELSE
    INSERT INTO public.product_prices (
      ...
      1,
      'prod_UvXKn9mQPp3G17',
      'price_1Tw6JqKuujVRKb0Pr4jCc5oQ',
      true
    );
  END IF;
END;
...
BEGIN
  ...
  WHERE product.product_key = 'aerocomms_pro'
    ...
    AND price.stripe_product_id = 'prod_UvXKn9mQPp3G17'
    AND price.stripe_price_id = 'price_1Tw6JqKuujVRKb0Pr4jCc5oQ'
  LIMIT 1;
  ...
  -- The commercial rule is one open AeroComms Pro subscription per account,
  -- including subscriptions on the archived 7.37 EUR price.
  IF EXISTS (
    SELECT 1
    FROM public.subscriptions AS subscription
    JOIN public.product_prices AS subscription_price
      ON subscription_price.id = subscription.product_price_id
    WHERE subscription.user_id = p_user_id
      AND subscription_price.product_id = v_product_id
  ...
  INSERT INTO public.orders (
    ...
    p_user_id, 'pending', 'EUR', 599, 0, 0, 599
  ) RETURNING id INTO v_order_id;

  INSERT INTO public.order_items (
    order_id, product_id, product_price_id, product_name, price_key, currency, unit_amount, quantity
  ) VALUES (
    v_order_id, v_product_id, v_product_price_id, v_product_name, v_price_key, 'EUR', 599, 1
  );
...
-- The public wrapper created by 20260712270000 keeps the exact 48-hour grace
-- semantics. Replace only its private projection implementation so a webhook
-- can resolve either immutable AeroComms Pro price identity.
CREATE OR REPLACE FUNCTION public.apply_aerocomms_pro_subscription_webhook_event_v1(
  p_event_id text,
  p_event_type text,
  p_payload_hash text,
...
DECLARE
  ...
  v_catalog_price_amount integer;
...
BEGIN
  ...
  IF v_action_requires_checkout THEN
    SELECT product.id, price.id, price.unit_amount, entitlement.id
      INTO v_product_id, v_catalog_price_id, v_catalog_price_amount, v_entitlement_id
    FROM public.products AS product
    JOIN public.product_prices AS price ON price.product_id = product.id
    ...
    WHERE product.product_key = 'aerocomms_pro'
      ...
      AND price.price_key IN ('aerocomms_pro_monthly_eur', 'aerocomms_pro_monthly_eur_599')
      AND price.currency = 'EUR'
      AND price.billing_type = 'recurring'
      AND price.billing_interval = 'month'
      AND price.interval_count = 1
      AND (
        (price.price_key = 'aerocomms_pro_monthly_eur'
          AND price.unit_amount = 737
          AND price.stripe_price_id = 'price_1TvgG4KuujVRKb0PkofwZMz7')
        OR
        (price.price_key = 'aerocomms_pro_monthly_eur_599'
          AND price.unit_amount = 599
          AND price.stripe_price_id = 'price_1Tw6JqKuujVRKb0Pr4jCc5oQ')
      )
    LIMIT 1;
  ...
  IF v_action_requires_checkout THEN
    ...
    SELECT attempt.user_id, attempt.order_id, attempt.stripe_checkout_session_id,
           order_row.user_id, order_row.total_amount, order_row.currency
      INTO v_attempt_user_id, v_attempt_order_id, v_attempt_session_id,
           v_order_user_id, v_order_total, v_order_currency
    FROM public.checkout_attempts AS attempt
    ...
    WHERE attempt.id = p_checkout_attempt_id
      ...
      AND item.product_price_id = v_catalog_price_id
    ...
    IF NOT FOUND
      ...
      OR v_order_total <> v_catalog_price_amount
    ...
    IF v_subscription_id IS NULL THEN
      ...
      UPDATE public.subscriptions
        SET user_id = p_user_id, order_id = p_order_id, product_price_id = v_catalog_price_id,
            ...
            grace_period_ends_at = CASE WHEN p_subscription_status = 'past_due' THEN grace_period_ends_at ELSE NULL END,
  ...
  ELSIF p_action = 'invoice_paid' THEN
    IF p_amount IS DISTINCT FROM v_catalog_price_amount
  ...
  ELSIF p_action = 'invoice_payment_failed' THEN
    ...
    UPDATE public.subscriptions
      SET status = 'past_due', last_provider_event_at = p_provider_created_at
      WHERE id = v_subscription_id;
