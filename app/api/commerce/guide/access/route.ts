...
import { COMO_SER_PILOTO_GUIDE_CHECKOUT_INTENT_COOKIE } from "@/lib/commerce/checkout";
import { isCommerceUuid } from "@/lib/commerce/contracts";
import {
  COMO_SER_PILOTO_GUIDE_DELIVERY_COOKIE,
  COMO_SER_PILOTO_GUIDE_DELIVERY_TOKEN_MAX_AGE_SECONDS,
  getCookieValue,
...
export async function POST(request: Request) {
  ...
  const checkoutIntentId = getCookieValue(request.headers.get("cookie"), COMO_SER_PILOTO_GUIDE_CHECKOUT_INTENT_COOKIE);
  if (!isCommerceUuid(checkoutIntentId)) return NextResponse.json(INVALID_REQUEST, { status: 403 });
  try {
    const delivery = await issueComoSerPilotoGuideDeliveryAccess(sessionId, checkoutIntentId);
    if (!delivery) return NextResponse.json(INVALID_REQUEST, { status: 403 });
    const response = NextResponse.json({ ok: true });
    response.cookies.set(COMO_SER_PILOTO_GUIDE_DELIVERY_COOKIE, delivery.token, {
      httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: "/",
      maxAge: Math.min(delivery.maxAge, COMO_SER_PILOTO_GUIDE_DELIVERY_TOKEN_MAX_AGE_SECONDS),
    });
    return response;
  } catch {
